...
class Help_Screen {
		...
		<ul style="list-style:disc;margin:0 0 1.5em 1.5em;max-width:55em;">
			...
			<li>
				...
				printf(
					...
					esc_html__( '%s — the row-by-row table: edit Score, Penalty or Exclude, and give a reason for the correction. Order above the table switches the rows between finishing position and surname (A–Z) — surname order is for checking the field against the start list, and re-ordering changes nothing, saves nothing and loses nothing you have already typed. Badges explain anything unusual: "failed upload" (a zero-score MapRun row, excluded from ranking but kept visible), "did not finish" (MapRun recorded no finish punch, which under the club\'s rule means no score, so it is excluded on import and kept visible too), "not in the latest import" (see the FAQ below), "added by hand", and "name not confirmed" (fix on Confirm names). An excluded row also says whether that runner already has a result scoring here, so putting one back is a decision rather than a guess. A highlighted row is one the warnings above name — scoring with no time recorded, run on a day other than the event\'s, or belonging to a runner who is scoring more than once. The last has no tick: two scoring rows for one runner is a mistake either way round, so the answer is always to exclude the rows that should not count. Check it against the night, then either Exclude it or tick "Checked — keep this row": both are recorded as corrections, and either clears the warning. They are alternatives, so the tick is not offered on a row you have excluded — un-exclude it and the tick comes back as you left it. A later import that changes that row\'s time, score, penalty, classifier or date brings the warning back, because the decision was about figures MapRun has since revised.', 'mvoc-streeto' ),
	...
	private function render_scoring(): void {
		...
		<ul style="list-style:disc;margin:0 0 1.5em 1.5em;max-width:55em;">
			...
			<li><?php esc_html_e( 'League points run 100 for first place down to 1 for hundredth, and 1 for anything below.', 'mvoc-streeto' ); ?></li>
